...
direct_declarator:
  ...
  | /* DF-C function declaration. */
    direct_declarator '(' parameter_list ';' parameter_list ')'
    {
      __has_dfc = 1;
      $$ = DfcFuncDecl($1, DfcMakeVars($3, $5));
    }
  | direct_declarator '(' ';' parameter_list ')'
    {
      __has_dfc = 1;
      $$ = DfcFuncDecl($1, DfcMakeVars(NULL, $4));
    }
  | direct_declarator '(' parameter_list ';' ')'
    {
      __has_dfc = 1;
      $$ = DfcFuncDecl($1, DfcMakeVars($3, NULL));
    }
  | direct_declarator '(' ';' ')'
    {
      $$ = DfcFuncDecl($1, DfcMakeVars(NULL, NULL));
    }
;
...
normal_function_definition:
    ...
    {
      if (isTypedef ||
          ($2->decl->type != DFUNC && /* DF-C function */ $2->decl->type != DDFCFUNC))
    ...
    compound_statement
    {
      ...
      {
        char *func_name = decl_getidentifier($2)->u.id->name;
  ...
  | declarator /* no return type */
    {
      if (isTypedef ||
          ($1->decl->type != DFUNC && /* DF-C function */ $1->decl->type != DDFCFUNC))
        parse_error(1, "function definition cannot be parsed.\n");
