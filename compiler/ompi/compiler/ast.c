...
astdecl DfcFuncDecl(astdecl decl, dfcvar var)
{
	astdecl d = Decl(DDFCFUNC, 0, decl, NULL);
	d->u.dfcvars = var;
	return (d);
}

dfcvar DfcMakeVars(astdecl inParams, astdecl outParams)
{
	dfcvar d = smalloc(sizeof(struct dfcvar_));
	d->inParams = inParams;
	d->outParams = outParams;
	return (d);
}
...
/* DF-C: new API to directly get identifier name of a astdecl. */
char *dfc_get_decl_identifier_name(astdecl node)
{
	while (node->type != IDENT)
	{
		node = node->decl;
	}
	return(node->u.id->name);
}
...
aststmt FuncDef(astspec spec, astdecl decl, aststmt dlist, aststmt body)
{
	...
	* **************************************************************/
	...
	if(decl->decl->type == DDFCFUNC)
	{
		// NOTE(luobin): Figure out how the initial process looks.
		dfc_all_df_function_tree = BlockList(dfc_all_df_function_tree, s);
	}
