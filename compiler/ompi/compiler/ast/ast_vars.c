...
static
void ast_declare_paramlist_vars(astdecl d, int is_dfc)
{
	/* This statement is for DF-C parsing rule purpose. It's a compromise. */
	if(!d)
		return;

	if (d->type == DLIST && d->subtype == DECL_paramlist)
	{
		ast_declare_paramlist_vars(d->u.next, is_dfc);
		d = d->decl;
	}
	if (d->type == DELLIPSIS)
		return;
	...
	if (d->decl != NULL && d->decl->type != ABSDECLARATOR)
	{
		...
		{
			char str[1024];
			snprintf(str, 1023, "DF_fanout_%s", s->name);
			symtab_insert_global(stab, Symbol(str), IDNAME);
		}
	}
}
	...
	if(d->type == DFUNC)
	{
		if ((d = d->u.params) != NULL)
			if (d->type == DPARAM || (d->type == DLIST && d->subtype == DECL_paramlist))
				ast_declare_paramlist_vars(d, 0);
	}
	else
	{
		astdecl in = d->u.dfcvars->inParams;
		astdecl out = d->u.dfcvars->outParams;
		ast_declare_paramlist_vars(in, 1);
		ast_declare_paramlist_vars(out, 2);
	}
}
