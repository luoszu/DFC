...
#else		
	sprintf(outfile, "%s_dfc.c", noext);
#endif

/* DF-C Header Insertion */
/*	char headers[] = "#include <pthread.h>\n#include \\\"threadpool.h\\\"\n#include \\\"dfc.h\\\"\n";
	sprintf(cmd, "echo \"%s\" > %s.tmp && cat %s >> %s.tmp && cp %s %s.bak && mv %s.tmp %s",
			headers, fname, fname, fname, fname, fname, fname, fname);
	if ((res = system(cmd)) != 0)
		_exit(res);
*/
	/* Preprocess
	...
	// sprintf(cmd, "mv %s.bak %s", fname, fname);
	// system(cmd);
...
int main(int argc, char **argv)
{
	...
	if (argc == 1)
	{
		...
		fprintf(stderr,
		        "\n"
		        "   OMPi options:\n"
		        "                  -s: the target code will recorde the schedule information\n" //ljr:-s选项生成的目标代码将会记录调度信息
